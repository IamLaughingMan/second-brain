...
注意：**hook 只 commit `wiki/ raw/ .vault-meta/`** —— `Meta/`、`.claude/`、`CLAUDE.md`、`Inbox/`、`Projects/`、`Bookmarks/`、`Personal/`、根層其他檔都**不在**範圍，需手動 commit。並且 hook **只 commit、不 push**（Claude bash 無 TTY；push 由你喺自己終端機做）。
