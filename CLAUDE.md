...
├── Personal/               # 個人念頭簿（root，獨立於 wiki；status 靠 frontmatter `stage`，不搬檔）
│   ├── Ideas.md              # hub（stage 詞彙 + promote→Projects 流程）
│   ├── Ideas.base            # view：00 全部 / 01 未做 / 02 做緊 / 03 完成 / 04 可能開project / Board
│   └── Ideas/                # 一念一檔：`type: idea` + `stage:`（未做/做緊/完成/可能開project）
...
│   └── ai-comms/                  # 跨-AI 溝通頻道（配 root AGENTS.md）
│       └── ai-comms.md              # protocol + append-only 訊息板（每則報名+HKT）
├── .claude/                  # Claude Code project config
│   ├── settings.json          # PostToolUse auto-commit hook（HKT，wiki/raw/.vault-meta）
│   └── skills/                # 自製 skills（Claude Code spec 寫死路徑；現空）
├── AGENTS.md                 # 俾其他 AI 自動讀：指佢哋睇 CLAUDE.md + Meta/ai-comms/，回應要報名
...
注意：**hook 只 commit `wiki/ raw/ .vault-meta/`** —— `Meta/`、`.claude/`、`CLAUDE.md`、`AGENTS.md`、`Inbox/`、`Projects/`、`Bookmarks/`、`Personal/`、根層其他檔都**不在**範圍，需手動 commit。並且 hook **只 commit、不 push**（Claude bash 無 TTY；push 由你喺自己終端機做）。

## 跨-AI 溝通（root `AGENTS.md` + `Meta/ai-comms/`）

本 vault 除咗 `CLAUDE.md`（Claude Code 專讀），仲有 root **`AGENTS.md`**（vendor-neutral，俾**其他** AI agent 如 Codex／Gemini／Cursor 自動讀）。`AGENTS.md` 指引其他 AI:① 跟佢入面**自含**嘅通用規矩 ② 去 **`Meta/ai-comms/ai-comms.md`** 溝通頻道讀/留言 ③ 回應時報上自己係乜 AI。

**`AGENTS.md` ⊂ `CLAUDE.md`（兩層拆分）**:`AGENTS.md` 嘅「Vault conventions」section = 本 `CLAUDE.md` **agent-agnostic 子集**（frontmatter／wikilink／log append-only／HKT／dual-log／status-over-folder／commit scope…），自含一份等其他 AI 唔使開 `CLAUDE.md` 都有齊核心規矩；`CLAUDE.md` 係 authoritative superset（多咗 Claude 專屬機制:hook／Skill／memory／RTK／Happy）。**同步責任**:若改動嘅係通用規矩（會影響任何 AI），**兩個檔都要 update**；純 Claude 專屬嘢只改 `CLAUDE.md`。衝突時 `CLAUDE.md` 為準。

Claude（即我）自己嘅責任:
- **想同其他 AI 溝通／交代／handoff** → 喺 `Meta/ai-comms/ai-comms.md` 嘅 `## Messages` **append**，每則第一行用 `[Claude / <model> · YYYY-MM-DD HH:MM HKT]` 報名（append-only、HKT）。
- Session 入面若見到其他 AI 喺嗰度留咗嘢 → 當 context 讀，回覆都 append 落去。
- `AGENTS.md`、`Meta/ai-comms/` 都喺 auto-commit hook scope **外** → 手動 commit。
