...
| **SessionEnd** | 寫 session-end CSV | `~/.claude/log-session-end.sh` |
...
| **Stop**（新 2026-06-19） | 每 turn 完即時 mirror 對話入 vault `Claude history/<YYYY-MM-DD>/<HH-MM project (prompt)>.md`（per-session .md、frontmatter `type: claude-session`、`<system-reminder>` 等 CLI noise 已 filter） | `python3 ~/.claude/hooks/log-claude-session.py` |
...
**Stop hook 細節**：state 喺 `~/.claude/state/claude-history-sessions.json`（`session_id → file_path/last_uuid/turn_count`）；錯誤 swallow 入 `~/.claude/state/claude-history-errors.log`，永不 block Claude Code。Hub／.base 喺 `[[Claude history]]`；session 內容由 `.gitignore` 排除（私隱）。
