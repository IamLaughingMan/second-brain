---
type: digest
category: Finance
date: 2026-06-23
sources:
  - https://news.cnyes.com/news/cat/headline
status: active
created: 2026-06-23
updated: 2026-06-23
tags:
  - digest
  - finance
  - 2026-06-23
---

# Finance Digest — 2026-06-23

> Source: [[鉅亨網 — 頭條新聞 cnyes（TW 金融全方位新聞 portal）|cnyes]] 頭條 snapshot；HK 時 ~19:00 抓取。

## TL;DR

- **比特幣跌穿 $62k**，晶片股重挫疊加**地緣政治緊張**（荷姆茲海峽）引發 risk-off 拋售。
- **韓股驚傳熔斷**、科技巨頭慘跌——亞太 risk asset 同步受壓。
- **AI 淘金熱擴散**：瑞銀點名台韓供應鏈，**台灣晶圓代工**＝「鏟子與圓鍬」beneficiary。
- 台股個股動態：嘉晶／大中／世芯點名 7 月潛力股；旭然 7/9 除息（0.6 元股息）。
- 監管／scandal：**壽險公司內鬼侵吞 1658 萬元 + 偷禮券**，金管會啟動金檢。

## Markets

### Crypto
- **比特幣跌至 $62k 防線**，地緣＋晶片股拖累；macro risk-off 主導。

### 亞太股市
- **韓股熔斷**（科技巨頭慘跌、追價意願創本月新低），但仍有兩檔逆勢鎖漲停（cnyes 標題未列名）。
- **台股**：法人預期 7 月反彈空間，點名嘉晶（碳化矽）、大中（功率半導體）、世芯（ASIC 設計服務）為潛力股。

### Macro／Geopolitics
- **荷姆茲海峽**緊張持續觸發**油價**＋風險資產 spillover；FED yield 觀察 ongoing。

## Equity 個股要點

| Ticker | 動態 | source |
|--------|------|--------|
| 旭然 (台股) | 7/9 除息 0.6 元股息，看好 2026 雙位數營收成長 | cnyes id 6509059 |
| 建通 (台股) | 越南廠新增高導電匯流銅排製程；轉型成效顯現 | cnyes id 6508644 |
| 三大未來 (台股) | 屏東新廠效益發揮，去年轉盈，每股配息 0.25 元 | cnyes id 6508988 |
| 嘉晶／大中／世芯 | 法人點名 7 月潛力 | cnyes id 6509064 |

## AI Supply Chain 焦點

**瑞銀（UBS）報告**：AI 淘金熱擴散，**台灣晶圓代工**（TSMC 為首）成最強「鏟子與圓鍬」beneficiary；**台韓供應鏈** broader pick：
- 半導體記憶體（美光／三星 / SK Hynix）
- AI PC（Computex 主題）
- 廣達等 server ODM

## Scandal / Regulatory

- **壽險公司內鬼 A 走懸帳 1658 萬元 + 偷禮券** → 金管會去金檢、依法裁處。Compliance / KYC governance 警號。

## 熱搜 tags（snapshot 反映 market 共識焦點）

`Computex` · `黃仁勳` · `AI PC` · `輝達 (Nvidia)` · `三星` · `廣達` · `記憶體` · `美光` · `殖利率` · `SpaceX` · `韓股` · `AMD` · `台積電` · `FED` · `半導體` · `荷姆茲海峽` · `油價` · `OpenAI` · `黃金` · `比特幣` · `日元` · `馬斯克` · `谷歌` · `AI` · `台幣` · `美元指數` · `蘋果` · `統一發票` · `ETF`

## My Take / Action Items

- **Risk-off setup**：BTC < $62k + 韓股熔斷 + 油價 spike → 短期 cash 比重 ↑；觀察是否觸發 [[Squeeze candidate watchlist — 掃 SI overflow 股做 short squeeze play|squeeze opportunities]]（leveraged shorts 喺急跌時最易被 short squeeze）。
- **AI supply chain**：TSMC／廣達／美光仍係 secular winner，但要留意短線跟隨 risk-off 嘅 mean revert；瑞銀報告利好 mid-term thesis。
- **台股**：嘉晶／大中／世芯 7 月 setup 值得 watchlist 加入；旭然 7/9 除息可注意 ex-div 後 reversion。

## Related

- [[Digest/2026-06-23/International|International 2026-06-23]]
- [[Digest/2026-06-23/AI|AI 2026-06-23]]
- [[鉅亨網 — 頭條新聞 cnyes（TW 金融全方位新聞 portal）]]
- [[Squeeze candidate watchlist — 掃 SI overflow 股做 short squeeze play]]
- [[TQQQ 10 年漲 7298% 但 WSJ 不建議貿然購買 槓桿衰減（風傳媒轉載 WSJ 2020）]]
